...
| AST-SCROLL-VIDEO | Decorative scroll-scene illustration (ink tree becoming a node network, day to night) | ES / EN | AI-generated for this site from prompts in `docs/design/phase-6/scroll-video/PROMPTS.md` | `/media/scroll/ink-tree-network-v2.mp4` | Approved by Luciano on 2026-09-14; served by the Phase 6 front (H.264 all-intra, 848×480, 81 frames, no audio, 888,519 bytes / ~868 KB); the same-frame v1 encode (246 KB) is kept in the repository unserved, per the versioning rule | Decorative (`aria-hidden`); the scene copy is available as text | No personal data, text, logos or people in the frames. |
| AST-SCROLL-POSTER | First frame of the scroll video, shown while it loads | ES / EN | Extracted from `AST-SCROLL-VIDEO` | `/media/scroll/ink-tree-network-v2-poster.webp` | Served by the Phase 6 front | Decorative | Same as `AST-SCROLL-VIDEO`. |
