...
| AST-SCROLL-VIDEO | Decorative scroll-scene illustration (ink tree becoming a node network, day to night) | ES / EN | V4 source video supplied by Luciano on 2026-09-22 (see `docs/design/phase-6/scroll-video/PROMPTS.md`); re-encoded only, no visual edits | `/media/scroll/ink-tree-network-v4.mp4` | Approved by Luciano on 2026-09-22; served by the front (H.264 all-intra, 1920×1080, 121 frames, 24 fps, no audio, 2,328,248 bytes / ~2.22 MB); v1, v2 and the previously served v3 (approved 2026-09-14, 1696×960, 2.16 MB) are kept unserved in `docs/design/phase-6/scroll-video/` (with their posters), per the versioning rule | Decorative (`aria-hidden`); the scene copy is available as text | No personal data, text, logos or people in the frames. |
| AST-SCROLL-POSTER | First frame of the scroll video, shown while it loads | ES / EN | Extracted from `AST-SCROLL-VIDEO` | `/media/scroll/ink-tree-network-v4-poster.webp` | Served by the front (frame 0, 1920×1080, 14,012 bytes) | Decorative | Same as `AST-SCROLL-VIDEO`. |
