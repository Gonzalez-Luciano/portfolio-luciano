...
class EditWorkCase extends EditRecord
{
        ...
        return Action::make('change_key')
            ...
            ->schema([
                TextInput::make('key')
                    ...
                    ->unique(table: 'work_cases', column: 'key', ignoreRecord: true),
            ])
     ...
     * and commits it together with the parent's own attributes in a single
    ...
    protected function handleRecordUpdate(Model $record, array $data): Model
    {
        ...
        if (! array_key_exists('technologies', $data)) {
            throw new \LogicException('The work case form did not submit its technologies state.');
        }
        ...
        // technologies is passed to the domain action as its own
        // parameter, not as a plain attribute.
        unset($data['technologies'], $data['key'], $data['key_locked'], $data['position'], $data['status'], $data['is_visible'], $data['published_at']);
