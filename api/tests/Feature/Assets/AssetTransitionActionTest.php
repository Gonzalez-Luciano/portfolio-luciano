...
final class AssetTransitionActionTest extends TestCase
{
    ...
    public function test_show_copies_a_published_hidden_owned_asset_only_after_the_owner_is_committed_visible(): void
    {
        ...
        $technology = app(PublishContent::class)($technology->fresh());

        $shown = app(ShowContent::class)($technology);

        $this->assertTrue($shown->is_visible);
        $this->assertNotNull($shown->icon_public_path);
        Storage::disk('public')->assertExists($shown->icon_public_path);
    }

    public function test_hide_and_return_to_draft_withdraw_the_public_copy_and_leave_no_public_reference(): void
    {
        $technology = $this->visibleTechnologyWithIcon();

        $hidden = app(HideContent::class)($technology);
        Storage::disk('public')->assertMissing($hidden->icon_public_path ?? 'missing');
        $this->assertFalse($hidden->is_visible);
        $this->assertNull($hidden->icon_public_path);
    ...
    {
        $technology = $this->visibleTechnologyWithIcon();
        $privatePath = $technology->icon_private_path;
        $publicPath = $technology->icon_public_path;

        app(DeleteContent::class)($technology);
    ...
    {
        $technology = $this->visibleTechnologyWithIcon();
        $oldPrivate = $technology->icon_private_path;
        $oldPublic = $technology->icon_public_path;
        ...
        try {
            app(AssetLifecycleService::class)->replace($technology, $this->png('replacement.png'));
            $this->fail('A failed public replacement must be controlled.');
        } catch (AssetOperationException) {
            $technology->refresh();
            $this->assertSame($oldPrivate, $technology->icon_private_path);
            $this->assertSame($oldPublic, $technology->icon_public_path);
    ...
    public function test_failed_public_copy_verification_restores_the_prior_reference_and_retains_the_old_public_copy(): void
    {
        ...
        try {
            app(AssetLifecycleService::class)->replace($technology, $this->png('unverifiable.png'));
    ...
    public function test_hide_withdraws_then_forgets_mutates_and_forgets_again_while_mutation_locks_remain_held(): void
    {
        ...
        Technology::updated(function () use ($store): void {
    ...
    {
        $technology = $this->visibleTechnologyWithIcon();
        $oldPath = $technology->icon_private_path;
        $local = Mockery::mock();
        $public = Storage::disk('public');
        ...
        try {
            app(RemoveOwnedAsset::class)($technology);
    ...
    public function test_post_commit_cache_failure_does_not_restore_the_withdrawn_public_copy(): void
    {
        ...
        // Technology invalidates 4 endpoints (Technologies, Experiences,
        // WorkCases, Projects) across 2 locales, so the pre-commit
        // invalidate() call alone consumes 8 forget() calls; failing from
        // the 9th is what lands the failure in the post-commit call.
        $store->failFromForget = 9;
    ...
    public function test_pre_commit_reduction_failure_restores_the_withdrawn_public_copy(): void
    {
        ...
        try {
            ...
            $technology->refresh();
            $this->assertTrue($technology->is_visible);
            $this->assertSame($publicPath, $technology->icon_public_path);
    ...
    public function test_earlier_throwing_after_commit_listener_does_not_restore_a_committed_reduction(): void
    {
        $technology = $this->visibleTechnologyWithIcon();
        $publicPath = $technology->icon_public_path;
        Technology::updated(static function (): void {
