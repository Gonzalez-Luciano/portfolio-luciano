          ...
          <ul>
            {links.map((link) => (
              <li key={link.key} className="border-b border-line first:border-t">
                {link.key === 'email' ? (
                  <a href={link.href} className={LINK_CLASS} data-umami-event={ANALYTICS_EVENTS.email}>
                    <span>
                  ...
                  <ExternalLink
                    href={link.href}
                    label={link.label}
                    newTab={ui.contact.newTab}
                    className={LINK_CLASS}
                    analyticsEvent={link.key === 'github' ? ANALYTICS_EVENTS.github : ANALYTICS_EVENTS.linkedin}
                  />
                )}
              </li>
            ))}
