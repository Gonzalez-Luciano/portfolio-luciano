...
// Hoisted once: the hill strokes never change with progress.
const HILLS = (
...
export function SceneBackdrop({ progress, reducedMotion }: { progress: number; reducedMotion: boolean }) {
  ...
  // Static geometry (cloud outlines, link lines, star marks) never changes across frames; memoizing
  // it once lets React reuse the same element references every render, so only the dynamic wrapper
  // (transform/opacity) is diffed instead of every point/path underneath it.
  const cloudPaths = useMemo(
    () =>
      CLOUDS.map((cloud) => (
        <path key={cloud.path} data-scene-cloud d={cloud.path} stroke="#665244" strokeOpacity={0.24} strokeWidth={1.5} />
      )),
    [],
  )

  const linkLines = useMemo(
    () => LINKS.map(([from, to]) => <line key={`${from}-${to}`} x1={STARS[from].x} y1={STARS[from].y} x2={STARS[to].x} y2={STARS[to].y} />),
    [],
  )

  const starMarks = useMemo(
    () =>
      STARS.map((star) => (
        <Fragment key={`${star.x}-${star.y}`}>
    ...
    >
      ...
      <g style={{ opacity: landscapeOpacity(progress) }} transform={`translate(0 ${landscapeDrift})`} fill="none" strokeLinecap="round">
        ...
        {CLOUDS.map((cloud, index) => (
          <g key={cloud.path} transform={`translate(${cloudOffset(progress, cloud.rate, reducedMotion)} 0)`}>
            {cloudPaths[index]}
          </g>
        ))}
      </g>
      ...
      <g style={{ opacity: moon.opacity }} fill="none" stroke="#E38B50" strokeOpacity={0.42} strokeWidth={1.5}>
        <path data-scene-moon d={`M ${moon.x} ${moon.y - 34} a 34 34 0 1 0 0 68 a 25 25 0 1 1 0 -68`} />
      </g>
